...
fn main() {
    ...
    println!("Init School: {:?}", school);

    // Test case 2:
    let mut school_2 = School::new();

    school_2.add(2, "Lee");
    school_2.add(3, "Nancy");

    println!("School's grades: {:?}", school_2.grades());

    // Test case 3:
    let mut school_3 = School::new();

    school_3.add(4, "Bob");
    school_3.add(4, "Alice");
    school_3.add(5, "Tom");

    println!("Students have grade 4: {:?}", school_3.find_students(4));
}
...
impl<T> School<T> where T: Copy + std::cmp::PartialEq + std::cmp::Ord {
    pub fn new() -> School<T> {
    ...
    pub fn find_students(&self, grade: T) -> Vec<String> {
